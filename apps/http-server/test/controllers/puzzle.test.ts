...
describe('Puzzle Controller', () => {
  ...
  beforeAll(async () => {
    ...
    const admin = await prisma.user.create({
      data: { email: `admin_puzzle_${Date.now()}@test.com`, username: `admin_puzzle_${Date.now()}`, isAdmin: true }
    });
    adminId = admin.id;
    ...
    const user = await prisma.user.create({
      data: { email: `user_puzzle_${Date.now()}@test.com`, username: `user_puzzle_${Date.now()}`, isAdmin: false }
    });
