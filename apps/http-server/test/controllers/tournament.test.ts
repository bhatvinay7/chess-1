...
describe('Tournament Controller', () => {
  ...
  beforeAll(async () => {
    ...
    // Create users
    const u1 = await prisma.user.create({ data: { email: `t1_${Date.now()}@test.com`, username: `tourney1_${Date.now()}` } });
    const u2 = await prisma.user.create({ data: { email: `t2_${Date.now()}@test.com`, username: `tourney2_${Date.now()}` } });
    user1Id = u1.id;
    user2Id = u2.id;
  ...
  afterAll(async () => {
    await prisma.tournamentParticipant.deleteMany({ where: { playerId: { in: [user1Id, user2Id] } } });
    await prisma.tournament.deleteMany({ where: { creatorId: { in: [user1Id, user2Id] } } });
  ...
  it('should get empty tournament rounds since it has not started', async () => {
    const res = await request(app).get(`/api/v1/tournaments/${tournamentId}/rounds`);
    expect(res.status).toBe(200);
  ...
  it('should delete the tournament', async () => {
    ...
    const getRes = await request(app).get(`/api/v1/tournaments/${tournamentId}`);
