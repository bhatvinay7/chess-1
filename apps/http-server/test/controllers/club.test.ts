...
describe('Club Controller', () => {
  ...
  beforeAll(async () => {
    // Create users
    const u1 = await prisma.user.create({ data: { email: `c1_${Date.now()}@test.com`, username: `club1_${Date.now()}` } });
    const u2 = await prisma.user.create({ data: { email: `c2_${Date.now()}@test.com`, username: `club2_${Date.now()}` } });
    user1Id = u1.id;
    user2Id = u2.id;
  ...
  afterAll(async () => {
    await prisma.clubJoinRequest.deleteMany({ where: { userId: { in: [user1Id, user2Id] } } });
    await prisma.clubMember.deleteMany({ where: { userId: { in: [user1Id, user2Id] } } });
    await prisma.club.deleteMany({ where: { creatorId: { in: [user1Id, user2Id] } } });
  ...
  it('should prevent duplicate club names', async () => {
    const res = await request(app)
      ...
      .set('Authorization', `Bearer ${token1}`)
      .send({
        name: await prisma.club.findUnique({ where: { id: clubId } }).then(c => c?.name),
  ...
  it('should allow admin to accept join request', async () => {
    ...
    expect(getRes.body.members.some((m: any) => m.userId === user2Id)).toBe(true);
  });
