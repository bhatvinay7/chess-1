...
describe("Friend Controller", () => {
  ...
  beforeAll(async () => {
    ...
    const u1 = await prisma.user.create({
      data: {
        email: `f1_${Date.now()}@test.com`,
        username: `friend1_${Date.now()}`,
      },
    });
    const u2 = await prisma.user.create({
      data: {
        email: `f2_${Date.now()}@test.com`,
        username: `friend2_${Date.now()}`,
      },
    });
    const u3 = await prisma.user.create({
      data: {
        email: `f3_${Date.now()}@test.com`,
        username: `friend3_${Date.now()}`,
      },
    });
  ...
  it("should reject a friend request", async () => {
    ...
    const requestId = incomingReq.body.requests.find(
      (r: any) => r.user.id === user3Id,
    ).id;
    ...
    const listRes = await request(app)
      ...
      .set("Authorization", `Bearer ${token2}`);
    expect(
      listRes.body.friends.find((f: any) => f.user.id === user3Id),
    ).toBeUndefined();
  });
});
