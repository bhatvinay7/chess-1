...
describe('Invite Controller', () => {
  ...
  beforeAll(async () => {
    ...
    // Create 3 users
    const u1 = await pgPrisma.user.create({ data: { email: `i1_${Date.now()}@test.com`, username: `inviter1_${Date.now()}` } });
    const u2 = await pgPrisma.user.create({ data: { email: `i2_${Date.now()}@test.com`, username: `inviter2_${Date.now()}` } });
    const u3 = await pgPrisma.user.create({ data: { email: `i3_${Date.now()}@test.com`, username: `inviter3_${Date.now()}` } });
  ...
  afterAll(async () => {
    ...
    const invites = await mongoPrisma.invitation.findMany({
      where: { OR: [{ senderId: user1Id }, { senderId: user2Id }, { senderId: user3Id }] }
    });
