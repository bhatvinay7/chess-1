...
export const TournamentAccessTypeSchema = z.enum(['CLUB', 'CROSS_CLUB', 'OPEN', 'PRIVATE']);
export const TournamentTypeSchema = z.enum(['CLUB_SWISS', 'CLUB_ROUND_ROBIN', 'GLOBAL_SWISS', 'GLOBAL_ROUND_ROBIN', 'DAILY', 'ARENA']);
export const TournamentVisibilitySchema = z.enum(['PUBLIC', 'PRIVATE', 'CLUB_ONLY']);
export const TournamentStatusSchema = z.enum(['DRAFT', 'REGISTRATION_OPEN', 'REGISTRATION_CLOSED', 'NOT_INITIALIZED', 'IN_PROGRESS', 'COMPLETED', 'CANCELLED']);
export const GameTypeSchema = z.enum(['STANDARD', 'CHESS960']);
export const TieBreakMethodSchema = z.enum(['BUCHHOLZ', 'MEDIAN_BUCHHOLZ', 'SONNEBORN_BERGER', 'DIRECT_ENCOUNTER', 'MOST_WINS']);
...
export const swissSettingsSchema = z.object({
  totalRounds: z.number({ error: "totalRounds is required for Swiss tournaments" }).int().positive(),
...
export const arenaSettingsSchema = z.object({
  durationMinutes: z.number({ error: "durationMinutes is required for Arena tournaments" }).int().positive(),
...
export const dailySettingsSchema = z.object({
  groupSize: z.number({ error: "groupSize is required for Daily tournaments" }).int().positive(),
  advancePerGroup: z.number({ error: "advancePerGroup is required for Daily tournaments" }).int().positive(),
  concurrentGamesPerOpponent: z.number().int().positive().default(2),
  daysPerMove: z.number({ error: "daysPerMove is required for Daily tournaments" }).int().positive(),
...
const SWISS_TYPES = ['CLUB_SWISS', 'GLOBAL_SWISS', 'CLUB_ROUND_ROBIN', 'GLOBAL_ROUND_ROBIN'] as const;

// Conditional validation pipeline using superRefine
export const tournamentSchema = baseTournamentSchema.superRefine((data, ctx) => {
  if ((SWISS_TYPES as readonly string[]).includes(data.tournamentType) && !data.swissSettings) {
    ctx.addIssue({
      code: 'custom',
  ...
  if (gapMin < 20) {
    ctx.addIssue({
      code: 'custom',
      message: 'Registration must close at least 20 minutes before the start time.',
  ...
  if (gapMin > 45) {
    ctx.addIssue({
      code: 'custom',
      message: 'Registration must close no more than 45 minutes before the start time.',
      path: ['registrationCloseAt'],
    });
  }
});
