...
export async function getUserProfile(req: Request, res: Response): Promise<void> {
    ...
    try {
        const user = await prisma.user.findUnique({
            where: { id: userId },
            select: {
                id: true, username: true, email: true, profileImageUrl: true, bio: true,
                rating: true, wins: true, losses: true, draws: true, createdAt: true,
            },
        });
        if (!user) {
        ...
        let ratings: { category: string; rating: number; wins: number; losses: number; draws: number }[] = [];
        try {
            ratings = await prisma.userRating.findMany({
                where: { userId },
                select: { category: true, rating: true, wins: true, losses: true, draws: true },
            });
        } catch {
...
export async function updateUserProfile(req: Request, res: Response): Promise<void> {
    const userId = req.params.userId;
    const { username, profileImageUrl, bio } = req.body as { username?: string; email?: string; profileImageUrl?: string; bio?: string };
    ...
    try {
        ...
        const updateKeys = {} as { username?: string; profileImageUrl?: string; bio?: string };
