...
export async function verifyOtp(req: Request, res: Response): Promise<void> {
  try {
    const { email, otp, username } = req.body as { email?: string; otp?: string; username?: string };
    ...
    if (!user) {
      ...
      const generatedUsername = username?.trim() || `${emailPrefix}_${Date.now()}`;
      user = await prisma.user.create({
        data: {
          username: generatedUsername, email, isAdmin: false,
          ratings: { create: [{ category: "BULLET" }, { category: "BLITZ" }, { category: "RAPID" }] },
        },
      });
    }
    ...
    res.json({
      token,
      user: { id: user.id, username: user.username, email: user.email ?? null, rating: user.rating },
    });
  } catch (error) {
...
export async function verifyLogin(req: Request, res: Response): Promise<void> {
  ...
  if (!user) {
    res.status(404).json({ message: "No account found for this email. Please sign up." });
    return;
  }
...
export async function loginWithPassword(req: Request, res: Response): Promise<void> {
  try {
    ...
    const credAuth = user.authMethods.find((a: (typeof user.authMethods)[number]) => a.authType === "CREDENTIALS");
    const passwordHash = credAuth?.passwordHash || user.password;

    if (!passwordHash) {
      res.status(401).json({ message: "Invalid email or password. If you signed up with Google, please use Google sign-in." });
      return;
    }
    ...
    const token = signToken({ userId: user.id, isAdmin: false });
    syncUserToMongo(user.id).catch(console.error);
    res.json({ token, user: { id: user.id, username: user.username, email: user.email ?? null, rating: user.rating } });
  } catch (error) {
    res.status(500).json({ message: error instanceof Error ? error.message : String(error) });
  }
}
...
export async function signupVerifyOtp(req: Request, res: Response): Promise<void> {
  try {
    const { email, otp, username, password } = req.body as {
      email?: string; otp?: string; username?: string; password?: string;
    };
    ...
    if (!result.valid) {
      res.status(401).json({
        message: result.expired ? "OTP has expired. Please request a new one." : "Invalid OTP",
      });
      return;
    }
...
export async function googleAuth(req: Request, res: Response): Promise<void> {
  try {
    const { idToken, email: bodyEmail, name: bodyName, picture: bodyPicture } = req.body as {
      idToken?: string;
      email?: string;
      name?: string;
      picture?: string;
    ...
    if (!sub || !email) {
      res.status(400).json({ message: "Google token did not contain required email and identity" });
      return;
    }
    ...
    let userAuth = await prisma.userAuth.findFirst({
      where: {
        OR: [
          { providerId: sub },
          { providerId: email, authType: "GOOGLE" },
        ],
      },
      include: { user: true },
    });
    ...
    if (!user) {
      ...
      if (user) {
        ...
        const emailPrefix = (email.split("@")[0] ?? "user").replace(/[^a-zA-Z0-9_]/g, "_");
        const generatedUsername = name?.replace(/[^a-zA-Z0-9_]/g, "_").slice(0, 20) || `${emailPrefix}_${Date.now()}`;
