...
export async function createTournament(req: Request, res: Response) {
  ...
  const isClubEvent =
    CLUB_TYPES.includes(d.tournamentType) || d.accessType === "CLUB";
  ...
  if (isClubEvent) {
    if (!d.clubId) {
      return res
        ...
        .json({
          success: false,
          message: "clubId is required for club events.",
        });
    }
    ...
    if (!membership || membership.role !== "ADMIN") {
      return res
        ...
        .json({
          success: false,
          message: "Only club admins can create club events.",
        });
    }
  }
  ...
  if (d.accessType === "CROSS_CLUB") {
    if (!d.clubId) {
      return res
        ...
        .json({
          success: false,
          message: "clubId is required for cross-club events.",
        });
    }
    const membership = await prisma.clubMember.findUnique({
      where: { clubId_userId: { clubId: d.clubId, userId } },
    ...
    if (!membership || membership.role !== "ADMIN") {
      return res
        ...
        .json({
          success: false,
          message: "Only club admins can create cross-club events.",
        });
    }
  }

  try {
    const newTournament = await prisma.$transaction(
    ...
    return res.status(500).json({
      ...
      errors: [
        { fieldName: "database", message: errorMessage || "Unknown error" },
      ],
    });
  }
}
