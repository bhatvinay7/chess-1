...
function resolveResult(gameState: string, winnerId: string | null | undefined, whiteId: string | null | undefined) {
  const win = gameState.includes("WHITE_WIN") || gameState === "RESIGN" && winnerId === whiteId
    ? "white"
    : gameState.includes("BLACK_WIN") || (gameState === "RESIGN" && winnerId && winnerId !== whiteId)
    ? "black"
    : null;

  const isDraw = !win && (
    gameState === "DRAW" || gameState === "STALEMATE" || gameState === "DRAW_BY_AGREEMENT" ||
    gameState === "GAME_OVER" || gameState === "CHECKMATE" // fallback — check winnerId
  );

  if (gameState === "CHECKMATE" && winnerId === whiteId) return { winner: "white" as const, label: "Checkmate" };
  if (gameState === "CHECKMATE" && winnerId)             return { winner: "black" as const, label: "Checkmate" };
  if (gameState.includes("WHITE_WIN"))                   return { winner: "white" as const, label: gameState.includes("TIMEOUT") ? "Timeout" : "Win" };
  if (gameState.includes("BLACK_WIN"))                   return { winner: "black" as const, label: gameState.includes("TIMEOUT") ? "Timeout" : "Win" };
  if (gameState === "RESIGN")                            return { winner: winnerId === whiteId ? "white" as const : "black" as const, label: "Resignation" };
  if (gameState === "STALEMATE")                         return { winner: null, label: "Stalemate" };
  if (gameState === "DRAW" || gameState === "DRAW_BY_AGREEMENT") return { winner: null, label: "Draw" };
...
function SpectatorGameOver({
  ...
  const winnerInfo = winner === "white" ? white : winner === "black" ? black : null;

  const emoji = isDraw ? "🤝" : "🏆";
  const title = isDraw ? "Draw" : `${winner === "white" ? "White" : "Black"} Wins!`;
    ...
    <div className={styles.gameOverOverlay}>
      <div className={styles.gameOverCard}>
        ...
        {winnerInfo && (
          <div className={styles.gameOverWinner}>
            <div className={styles.gameOverWinnerAvatar}>
              <img src={winnerInfo.img || DEFAULT_AVATAR} alt={winnerInfo.name} crossOrigin="anonymous" />
            </div>
            {winnerInfo.name}
          </div>
        )}
        ...
        <div className={styles.gameOverActions}>
          <button type="button" className={styles.gameOverAnalyse} onClick={onAnalyse}>
            Analyse
          </button>
          <button type="button" className={styles.gameOverDismiss} onClick={onDismiss}>
            Dismiss
          </button>
        </div>
      </div>
...
function SpectatorPlayerBar({
  ...
  return (
    <div className={`${styles.playerBar} ${position === "top" ? styles.playerBarTop : styles.playerBarBottom}`}>
      <div className={styles.playerLeft}>
        ...
        <div className={styles.playerMiddle}>
          <span className={styles.playerName}>
            {name}
            <span className={styles.playerRatingInline}> ({rating ?? "—"})</span>
          </span>
        </div>
      </div>
...
export function SpectatorBoard({ gameId }: SpectatorBoardProps) {
  ...
  const { spectateState, movePairs, moveCount, serverTimes, spectatorCount, connected, gameRef } =
  ...
  const chessTurn = useMemo(() => {
    try { return new Chess(currentFen).turn(); } catch { return "w" as const; }
  }, [currentFen]);
  const isWhiteToMove = chessTurn === "w";

  // Clock — spectators view only, no timeout callback
  const { displayWhiteTime, displayBlackTime, syncTimeFromOutside } = useChessTimer(
  ...
  const white = {
    name:   isP1White ? (spectateState?.player1Username ?? "White") : (spectateState?.player2Username ?? "White"),
    rating: isP1White ?  spectateState?.player1Rating               : spectateState?.player2Rating,
    img:    isP1White ?  spectateState?.player1ProfileImageUrl       : spectateState?.player2ProfileImageUrl,
  };
  const black = {
    name:   isP1White ? (spectateState?.player2Username ?? "Black") : (spectateState?.player1Username ?? "Black"),
    rating: isP1White ?  spectateState?.player2Rating               : spectateState?.player1Rating,
    img:    isP1White ?  spectateState?.player2ProfileImageUrl       : spectateState?.player1ProfileImageUrl,
  };
