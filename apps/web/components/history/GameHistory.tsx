...
export default function GameHistory() {
  ...
  const { data, isLoading, isFetching, error } = useGameHistory(page, PAGE_SIZE, filter);

  const games        = data?.games        ?? [];
  const total        = data?.total        ?? 0;
  const totalPages   = data?.totalPages   ?? 1;
  const filterCounts = data?.filterCounts ?? { all: 0, wins: 0, losses: 0, draws: 0, rated: 0 };
  const summary      = data?.summary      ?? { totalGames: 0, winRate: 0, reviewedGames: 0, averageAccuracy: null };
  ...
  return (
    <main className={styles.container}>
      ...
      {error && <section className={`glass-panel ${styles.errorState}`}>Could not load game history.</section>}

      {!isLoading && !error && (
        <div style={{ opacity: isFetching ? 0.55 : 1, transition: "opacity 0.15s" }}>
          ...
          {totalPages > 1 && (
            <div className={styles.pagination}>
              <span className={styles.paginationInfo}>
                Showing <strong>{startItem}–{endItem}</strong> of <strong>{total}</strong> games
              </span>
