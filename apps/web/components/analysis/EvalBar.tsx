...
export function EvalBar({ evalWhite }: { evalWhite: number }) {
  ...
  const display =
    abs >= 10000
      ? "M"
      : abs >= 1000
        ? `${(evalWhite / 100).toFixed(0)}`
        : `${(evalWhite / 100).toFixed(1)}`;

  return (
    <div className={styles.evalBarCol}>
      <div
        className={styles.evalBarBlack}
        style={{ height: `${100 - whitePct}%` }}
      />
      <div className={styles.evalBarFill} style={{ height: `${whitePct}%` }} />
      {evalWhite >= 0 ? (
        <span className={`${styles.evalLabel} ${styles.evalLabelTop}`}>
          {display}
        </span>
      ) : (
        <span className={`${styles.evalLabel} ${styles.evalLabelBot}`}>
          {display.replace("-", "")}
        </span>
      )}
    </div>
