...
import { Activity, BarChart3, ChevronLeft, ChevronRight, Edit2, Loader2, Swords, Trophy, Zap, Flame, Clock } from "lucide-react";
import { useAuth } from "../../hooks/useAuth";
import { useProfile, useUpdateProfile } from "../../hooks/useProfile";
import { useGameHistory, useRatingHistoryByCategory } from "../../hooks/useGameHistory";
import { uploadFile } from "../../app/lib/api/upload";
import type { RatingPoint } from "../../app/lib/api/games";
import { FriendsPanel } from "../social/FriendsPanel";

const DEFAULT_AVATAR = "https://res.cloudinary.com/dhfyav4og/image/upload/v1779739162/defaultUser_afbf5y.jpg";
const PAGE_SIZE = 8;

type ProfileTab = "Overview" | "Games" | "Stats" | "Friends" | "Awards" | "Clubs";
const TABS: ProfileTab[] = ["Overview", "Games", "Stats", "Friends", "Awards", "Clubs"];
...
function CategoryRatingChart({
  ...
  const W = 560, H = 130, PX = 28, PY = 14;
  const color = CAT_COLOR[category] ?? "#81b64c";
  const gradId = CAT_GRAD_ID[category] ?? "ratingGrad";

  if (points.length < 2) {
    return (
      <div style={{ padding: "1.2rem", textAlign: "center", color: "var(--text-faint)", fontSize: "0.8rem" }}>
  ...
  const area = `M ${toX(0)},${toY(firstRating)} ` +
  ...
  return (
    <div>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "0.4rem" }}>
        <span style={{ display: "flex", alignItems: "center", gap: "0.35rem", fontSize: "0.73rem", fontWeight: 700, color: "var(--text-faint)", textTransform: "uppercase", letterSpacing: "0.07em" }}>
          {CAT_ICON[category]}
          {category.charAt(0) + category.slice(1).toLowerCase()} · {points.length} rated games
        </span>
        <span style={{ fontSize: "0.78rem", color: deltaColor, fontWeight: 700 }}>
          {delta >= 0 ? "+" : ""}{delta} pts
        </span>
      </div>
      <svg viewBox={`0 0 ${W} ${H}`} width="100%" style={{ overflow: "visible", display: "block" }} aria-label={`${category} rating history`}>
        <defs>
        ...
        {yLabels.map(({ y, value }) => (
          <g key={value}>
            ...
            <text x={PX - 6} y={y + 4} textAnchor="end" fill="rgba(255,255,255,0.22)" fontSize="9">{value}</text>
          </g>
        ))}
        <path d={area} fill={`url(#${gradId})`} />
        <polyline points={polyline} fill="none" stroke={color} strokeWidth="2" strokeLinejoin="round" strokeLinecap="round" />
        {points.map((p, i) => {
          const show = i === 0 || i === points.length - 1 || (points.length > 10 && i % Math.max(1, Math.floor(points.length / 8)) === 0);
          if (!show) return null;
          const cx = toX(i);
          ...
          return (
            <g key={i}>
              <circle cx={cx} cy={cy} r={isLast ? 5 : 3} fill={isLast ? "#f28b38" : color} stroke="#1a1a1a" strokeWidth="1.5" />
              {isLast && (
                <text x={cx} y={cy - 10} textAnchor="middle" fill="#f28b38" fontSize="11" fontWeight="700">{p.rating}</text>
              )}
            </g>
          );
        })}
...
function RatingHistoryCharts() {
  ...
  const [activeTab, setActiveTab] = useState<"BULLET" | "BLITZ" | "RAPID">("BLITZ");

  if (isLoading) {
    return (
      <div style={{ display: "flex", alignItems: "center", gap: "0.5rem", padding: "1.5rem", color: "var(--text-faint)", fontSize: "0.82rem" }}>
        <Loader2 size={14} style={{ animation: "spin 1s linear infinite" }} /> Loading rating history…
      </div>
    );
  }
  ...
  return (
    <div>
      <div style={{ display: "flex", gap: "0.25rem", marginBottom: "0.85rem", borderBottom: "1px solid var(--card-border)", paddingBottom: "0" }}>
        {(["BULLET", "BLITZ", "RAPID"] as const).map((cat) => (
          ...
          >
            ...
            <span style={{ fontSize: "0.68rem", fontWeight: 400, color: activeTab === cat ? CAT_COLOR[cat] : "var(--text-faint)" }}>
              ({data?.[cat]?.length ?? 0})
            </span>
          </button>
        ))}
      </div>
      <CategoryRatingChart category={activeTab} points={data?.[activeTab] ?? []} />
    </div>
  );
}
...
function GameHistoryTable({ userId }: { userId: string }) {
  ...
  return (
    <div style={{ opacity: isFetching ? 0.6 : 1, transition: "opacity 0.15s" }}>
      {/* Header row */}
      <div className={styles.historyRow} style={{ opacity: 0.5, fontSize: "0.75rem", borderBottom: "1px solid var(--card-border)", paddingBottom: "0.5rem", marginBottom: "0.25rem" }}>
        <span>Opponent</span>
        <span>Result</span>
        <span>Accuracy</span>
        <span>Moves</span>
      ...
      {games.map((game) => {
        const accuracy = game.playerColor === "white"
          ? game.analysis?.whiteAccuracy
          : game.analysis?.blackAccuracy;

        const resultColor =
          game.result === "WIN" ? "#4ADE80" :
          game.result === "LOSS" ? "#F87171" :
          "#A78BFA";

        return (
          <div key={game.id} className={styles.historyRow} style={{ fontSize: "0.84rem", alignItems: "center" }}>
            <span style={{ color: "var(--text-primary)", fontWeight: 600 }}>
              {game.opponent.username ?? "Unknown"}
              <span style={{ fontSize: "0.72rem", color: "var(--text-faint)", fontWeight: 400, marginLeft: "0.4rem" }}>
                ({game.gameName} · {game.timeControl})
              </span>
            </span>
            <span style={{ color: resultColor, fontWeight: 800, textTransform: "capitalize" }}>
              {game.result === "WIN" ? "Win" : game.result === "LOSS" ? "Loss" : game.result === "DRAW" ? "Draw" : "—"}
            </span>
            <span style={{ color: "var(--text-muted)" }}>
              {accuracy != null ? `${accuracy.toFixed(1)}%` : "—"}
            </span>
            <span style={{ color: "var(--text-muted)" }}>{game.moveCount}</span>
            <span style={{ color: "var(--text-faint)" }}>{formatDate(String(game.date))}</span>
          </div>
        );
      })}
...
const paginationStyles: Record<string, React.CSSProperties> = {
  ...
  btn: {
    ...
    borderRadius: "8px", color: "var(--text-muted)", fontSize: "0.8rem",
    fontWeight: 600, padding: "0.35rem 0.75rem", cursor: "pointer",
...
export default function ProfileCard() {
  ...
  const handleUpdate = (updatedData: { username?: string; profileImageUrl?: string; bio?: string }) => {
    if (user?.id) {
    ...
    <div className={styles.container}>
      ...
      <div className={styles.profileBanner}>
        ...
        >
          ...
          {avatarError && (
            <p style={{ position: "absolute", bottom: "-1.4rem", left: 0, fontSize: "0.72rem", color: "#F87171", whiteSpace: "nowrap" }}>
              {avatarError}
            </p>
          )}
        </div>
        ...
        <div className={styles.bannerInfo}>
          {isEditing ? (
            ...
            <div className={styles.usernameRow}>
              ...
              <Edit2
                size={17}
                onClick={(e) => { e.stopPropagation(); setIsEditing(true); setUsername(profile.username); }}
          ...
          {editingBio ? (
            <div style={{ display: "flex", flexDirection: "column", gap: "0.5rem", maxWidth: "420px" }}>
              ...
              <div style={{ display: "flex", gap: "0.5rem" }}>
                <button
                  onClick={() => { handleUpdate({ bio }); setEditingBio(false); }}
                  disabled={updateProfileMutation.isPending}
                  className={styles.saveBtn}
                >
                  {updateProfileMutation.isPending ? "Saving…" : "Save Bio"}
                </button>
                <button onClick={() => setEditingBio(false)} className={styles.cancelBtn}>Cancel</button>
              </div>
            </div>
          ) : (
            <p
              className={styles.statusText}
              onClick={() => { setBio(profile.bio ?? ""); setEditingBio(true); }}
              style={{ cursor: "pointer" }}
              title="Click to edit bio"
            >
              {profile.bio ? profile.bio : <span style={{ opacity: 0.4, fontStyle: "italic" }}>Add a bio…</span>}
              <Edit2 size={12} style={{ marginLeft: "0.4rem", opacity: 0.4, verticalAlign: "middle" }} />
            </p>
          )}

          <div className={styles.metaRow}>
            <span className={styles.metaItem}>Joined {formatJoinDate(profile.createdAt)}</span>
      ...
      {activeTab === "Overview" && (
        <div className={styles.overviewGrid}>
          ...
          <div className={styles.ratingsRow}>
            {(["BULLET", "BLITZ", "RAPID"] as const).map((cat) => {
              const r = (profile as any).ratings?.find((x: any) => x.category === cat);
              ...
              return (
                <div key={cat} className={styles.ratingCatCard} style={{ "--cat-color": color } as React.CSSProperties}>
                  <div className={styles.ratingCatHeader}>
                    <span className={styles.ratingCatIcon}>{CAT_ICON[cat]}</span>
                    <span className={styles.ratingCatLabel}>{cat.charAt(0) + cat.slice(1).toLowerCase()}</span>
                  </div>
                  <div className={styles.ratingCatValue} style={{ color }}>{rating}</div>
                  <div className={styles.ratingCatBar}>
                    <div className={styles.ratingCatBarFill} style={{ width: ratingBarWidth(rating), background: color }} />
                  </div>
                  <div className={styles.ratingCatStats}>
                    ...
                    {total > 0 && <span className={styles.ratingCatTotal}>({total})</span>}
                  </div>
                </div>
              );
            })}
          ...
          <div className={styles.statsGrid}>
            <div className={styles.statCard}>
              ...
              <div>
                ...
                <div className={styles.statValue}>{profile.wins}W · {profile.losses}L · {profile.draws}D</div>
              </div>
            </div>
            <div className={styles.statCard}>
            ...
            <div className={styles.statCard}>
              ...
              <div>
                ...
                <div className={styles.statValue}>{winRate(profile.wins, profile.losses)}</div>
              </div>
            </div>
            <div className={styles.statCard}>
          ...
          <div className={styles.sectionCard}>
            <div className={styles.sectionHeader}>
              <Trophy size={18} />
              <span className={styles.sectionTitle}>Game History ({totalGames})</span>
            </div>
