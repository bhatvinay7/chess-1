...
import type { TournamentListItem, CreateTournamentPayload, TournamentType, TournamentStatus } from "./types";
...
export default function TournamentPage() {
  ...
  const [tabData, setTabData] = useState<Partial<Record<"current" | "future" | "ended", TournamentListItem[]>>>({});
  ...
  const loadTab = useCallback(async (targetTab: "current" | "future" | "ended") => {
    ...
    try {
      ...
      const [tabResult, mineResult] = await Promise.all([
        needsTab  ? fetchTournaments({ statuses: TAB_STATUSES[targetTab] }) : Promise.resolve(null),
        needsMine ? fetchMyTournaments().catch(() => null)                   : Promise.resolve(null),
      ]);
      if (tabResult) {
        loadedTabsRef.current.add(targetTab);
        setTabData((prev) => ({ ...prev, [targetTab]: tabResult.data ?? [] }));
      }
  ...
  const applyFilters = (list: TournamentListItem[]) =>
    list.filter((t) => {
      ...
      if (formatFilter === "ARENA" && t.tournamentType !== "ARENA") return false;
      if (
        formatFilter === "SWISS" &&
    ...
    <div className={styles.page}>
      ...
      <header className={styles.header}>
        <div className={styles.headerLeft}>
          ...
          <span className={styles.headerMeta}>{dateStr} {timeStr}</span>
        </div>
      </header>
      ...
      <div className={styles.content}>
        {(tab === "current" || tab === "future" || tab === "ended") && (
          <TournamentList
            ...
            now={now}
            emptyLabel={
              tab === "ended"   ? "No ended tournaments"    :
              tab === "current" ? "No live tournaments"     :
                                  "No upcoming tournaments"
            }
            emptySub={
              tab === "ended"   ? "Completed and cancelled tournaments appear here once they finish." :
              tab === "current" ? "No tournaments are in progress right now."                        :
...
function ScheduleView({ tournaments, now }: { tournaments: TournamentListItem[]; now: Date }) {
    ...
    <div className={styles.scheduleList}>
      {Array.from(byDay.entries()).map(([dateKey, dayTournaments]) => {
        ...
        for (const t of dayTournaments) {
          const slotKey = new Date(t.timeManagement!.startTime).toLocaleTimeString("en-US", {
            hour: "numeric",
            hour12: true,
          });
        ...
        return (
          <div key={dateKey} className={styles.slotDay}>
            <div className={styles.slotDayHeader}>
              <span className={styles.slotDayLabel}>{dayLabel(dayDate, now)}</span>
                    ...
                    return (
                      <div key={t.id} className={styles.slotCard}>
                        ...
                        <span
                          ...
                          style={{ color: isLive ? "#81b64c" : cfg?.color ?? "#7a7673" }}
                        >
                          {isLive ? "Live" : t.status.replace(/_/g, " ").toLowerCase()}
                        </span>
                      </div>
                    );
                  })}
...
function CreateView({ onOpen }: { onOpen: (type: TournamentType) => void }) {
  return (
    <div className={styles.createView}>
      <p className={styles.createPrompt}>Please select the kind of event you&apos;d like to create:</p>
      <div className={styles.createGroups}>
        {CREATE_SECTIONS.map((section) => (
          <div key={section.id} className={styles.createGroup}>
            {/* Section header */}
            <div className={styles.createGroupHeader} style={{ borderLeftColor: section.accentColor }}>
              <span className={styles.createGroupLabel} style={{ color: section.accentColor }}>
                {section.label}
              </span>
            ...
            <div className={styles.createOptions}>
              {section.options.map((opt) => {
                const cfg = TOURNAMENT_TYPE_CONFIGS.find((c) => c.id === opt.type);
                return (
                  <button
                    key={opt.type}
                    type="button"
                    ...
                    style={{ "--opt-color": cfg?.color ?? section.accentColor } as React.CSSProperties}
                  >
                    <span
                    ...
                    <div className={styles.createOptionBody}>
                      <span className={styles.createOptionLabel}>{opt.label}</span>
