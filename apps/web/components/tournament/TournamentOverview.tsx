      ...
      <div className={styles.card}>
        ...
        <div className={styles.rows}>
          <Row
            icon={<Users size={14} />}
            label="Players"
            value={`${t.participantCount}${t.maxPlayers ? ` / ${t.maxPlayers}` : ""}`}
          />
          <Row
            icon={<Trophy size={14} />}
            label="Format"
            value={typeConfig?.label ?? t.tournamentType}
          />
          <Row
            icon={<CalendarDays size={14} />}
            label="Starts"
            value={fmt(t.timeManagement.startTime)}
          />
          <Row
            icon={<Clock size={14} />}
            label="Registration Opens"
            value={fmt(t.timeManagement.registrationOpenAt)}
          />
          <Row
            icon={<Clock size={14} />}
            label="Registration Closes"
            value={fmt(t.timeManagement.registrationCloseAt)}
          />
          {t.timeManagement.endTime && (
            <Row
              icon={<Clock size={14} />}
              label="Ends"
              value={fmt(t.timeManagement.endTime)}
            />
          )}
          <Row
            icon={<Star size={14} />}
            label="Rated"
            value={t.isRated ? "Yes" : "No"}
          />
          ...
          {t.inviteOnly && (
            <Row icon={<Lock size={14} />} label="Invite Only" value="Yes" />
          )}
          {t.premiumOnly && (
            <Row icon={<Shield size={14} />} label="Premium Only" value="Yes" />
          )}
          {t.timeControl && (
            <Row
              icon={<Clock size={14} />}
              label="Time Control"
              value={t.timeControl.label}
            />
          )}
        </div>
      </div>
