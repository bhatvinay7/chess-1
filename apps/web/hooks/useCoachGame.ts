...
export function useCoachGame(config: CoachGameConfig | null) {
  ...
  const [lastMoveAnnotation, setLastMoveAnnotation] =
    useState<MoveAnnotation | null>(null);
  ...
  const bookCallbacksRef = useRef(
    new Map<number, (hints: { move: string; name: string }[]) => void>(),
  );
    ...
    worker.onmessage = (e: MessageEvent) => {
      ...
      if (msg.type === "error") {
        ...
        queued.forEach(({ id }) => {
          ...
          if (cb) {
            bookCallbacksRef.current.delete(id);
            cb([]);
          }
        });
        return;
      ...
      if (msg.type === "hints" && msg.id != null) {
        ...
        if (cb) {
          bookCallbacksRef.current.delete(msg.id);
          cb(msg.hints ?? []);
        }
      }
    };
  ...
  const applyEngineUci = useCallback(
    (uci: string) => {
      const game = gameRef.current;
      try {
        game.move({
  ...
  const applyEngineSan = useCallback(
    (san: string, openingName: string): boolean => {
      const game = gameRef.current;
      try {
        game.move(san);
      } catch {
  ...
  const fetchBookHints = useCallback(
    (currentFen: string) => {
      queryBook(currentFen, (hints) => {
        ...
        const parsed: BookHint[] = hints.flatMap((h) => {
          try {
            ...
            game.undo();
            return [
              { from: r.from, to: r.to, san: r.san, openingName: h.name },
            ];
          } catch {
            return [];
          }
