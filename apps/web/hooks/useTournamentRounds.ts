...
function computeStandingsFromMatches(
  ...
  for (const p of players) {
    map.set(p.id, { playerId: p.id, username: p.username, score: 0, wins: 0, draws: 0, losses: 0, byes: 0 });
  }
  for (const m of matches) {
    ...
    if (!w || !b) continue;
    if (m.status === "WHITE_WIN")  { w.wins++;  w.score += 0.5; b.losses++; }
    else if (m.status === "BLACK_WIN") { b.wins++;  b.score += 0.5; w.losses++; }
    else if (m.status === "DRAW")  { w.draws++; w.score += 0.5; b.draws++;  b.score += 0.5; }
  }
  return [...map.values()].sort((a, b) => b.score - a.score);
}

function apiMatchToUnified(m: TournamentRoundDetail["groups"][number]["matches"][number]): UnifiedMatch {
  return {
    matchId:  m.matchId,
    gameId:   m.gameId,
    gameState: m.gameState ?? m.status,
    white:    m.whitePlayer
      ? { id: m.whitePlayer.id, username: m.whitePlayer.username, profileImageUrl: m.whitePlayer.profileImageUrl ?? null }
      : null,
    black:    m.blackPlayer
      ? { id: m.blackPlayer.id, username: m.blackPlayer.username, profileImageUrl: m.blackPlayer.profileImageUrl ?? null }
      : null,
    winnerId:         m.winnerId,
...
export function useTournamentRounds({ tournamentId, liveData, isLive, active }: Params): Return {
  ...
  const rounds = useMemo<UnifiedRound[]>(() => {
    ...
    return apiRounds.map((apiRound): UnifiedRound => {
      ...
      const socketRound = liveData?.rounds.find(r => r.roundId === apiRound.roundId);

      const groups: UnifiedGroup[] = apiRound.groups.map((apiGroup): UnifiedGroup => {
        const matches = apiGroup.matches.map(apiMatchToUnified);

        const doneCount = matches.filter(m =>
          ["WHITE_WIN", "BLACK_WIN", "DRAW", "ABANDONED"].includes(m.gameState),
        ).length;
        ...
        const socketGroup = socketRound?.groups.find(g => g.groupId === apiGroup.groupId);
      ...
      const roundStandings: UnifiedRoundStanding[] = socketRound?.roundStandings.map(rs => ({
