...
describe("DrawHandler", () => {
  ...
  it("should handle offer-draw", async () => {
    const offerDrawCall = (socket.on as jest.Mock).mock.calls.find(
      (call: any[]) => call[0] === "offer-draw",
    );
  ...
  it("should handle claim-draw", async () => {
    const claimDrawCall = (socket.on as jest.Mock).mock.calls.find(
      (call: any[]) => call[0] === "claim-draw",
    );
  ...
  it("should handle decline-draw", async () => {
    const declineDrawCall = (socket.on as jest.Mock).mock.calls.find(
      (call: any[]) => call[0] === "decline-draw",
    );
    ...
    expect(PubSub.publish).toHaveBeenCalledWith(
      "decline-draw:game_1",
      JSON.stringify({ action: "declined" }),
    );
  });

  it("should handle check_draw_offer", async () => {
    const checkDrawOfferCall = (socket.on as jest.Mock).mock.calls.find(
      (call: any[]) => call[0] === "check_draw_offer",
    );
