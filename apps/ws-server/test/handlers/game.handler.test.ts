...
describe("GameHandler", () => {
  ...
  it("should handle user_move", async () => {
    const gameMoveCall = (socket.on as jest.Mock).mock.calls.find(
      (call: any[]) => call[0] === "user_move",
    );
  ...
  it("should handle search_opponent", async () => {
    const searchOpponentCall = (socket.on as jest.Mock).mock.calls.find(
      (call: any[]) => call[0] === "search_opponent",
    );
    ...
    expect(redisClient.set).toHaveBeenCalledWith("presence:user_a", "1", {
      EX: 30,
      NX: true,
    });
  });
  ...
  it("should handle join_arena", async () => {
    const joinArenaCall = (socket.on as jest.Mock).mock.calls.find(
      (call: any[]) => call[0] === "join_arena",
    );
    const joinArenaCb = joinArenaCall[1];

    (getActiveGameId as jest.Mock).mockResolvedValue("game_1");
    (syncGameTime as jest.Mock).mockResolvedValue({
      current_fen: "fen",
    ...
    expect(mockUserSocket.emit).toHaveBeenCalledWith("game_state", {
      state: "mock",
    });
  });

  it("should handle leave_search", async () => {
    const leaveSearchCall = (socket.on as jest.Mock).mock.calls.find(
      (call: any[]) => call[0] === "leave_search",
    );
