...
describe('RematchHandler', () => {
  ...
  it('should handle rematch-request', async () => {
    const rematchCall = (socket.on as jest.Mock).mock.calls.find((call: any[]) => call[0] === 'rematch-request');
    ...
    expect(requestRematch).toHaveBeenCalledWith({ userId: 'user_a', opponentId: 'user_b' });
    expect(PubSub.publish).toHaveBeenCalledWith('rematch-request:user_b', JSON.stringify({ action: 'allowed' }));
  });

  it('should handle accept-rematch', async () => {
    const acceptCall = (socket.on as jest.Mock).mock.calls.find((call: any[]) => call[0] === 'accept-rematch');
    ...
    expect(exceptRematch).toHaveBeenCalledWith({ userId: 'user_a', gameId: 'game_1' });
  });

  it('should handle decline-rematch-request', async () => {
    const declineCall = (socket.on as jest.Mock).mock.calls.find((call: any[]) => call[0] === 'decline-rematch-request');
    expect(declineCall).toBeDefined();
    const declineCb = declineCall[1];

    (declineRematchRequest as jest.Mock).mockResolvedValue({ action: 'declined' });

    await declineCb({ userId: 'user_a', gameId: 'game_1' });

    expect(declineRematchRequest).toHaveBeenCalledWith({ userId: 'user_a', gameId: 'game_1' });
  });

  it('should handle check-rematch-request', async () => {
    const checkCall = (socket.on as jest.Mock).mock.calls.find((call: any[]) => call[0] === 'check-rematch-request');
    expect(checkCall).toBeDefined();
    const checkCb = checkCall[1];

    (checkPendingRematchRequest as jest.Mock).mockResolvedValue({ status: 'pending' });
    ...
    expect(mockUserSocket.emit).toHaveBeenCalledWith('rematch-request', { status: 'pending' });
  });
});
