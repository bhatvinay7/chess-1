...
import { handleWatchGame, handleLeaveSpectate, syncSpectateTime } from '../../src/utils/spectateHandler.js';
import { spectatorGameMap, socketSpectatingMap } from '../../src/shared/socket-store.js';
...
describe('SpectateHandler', () => {
  ...
  it('should handle watch_game', async () => {
    const watchCall = (socket.on as jest.Mock).mock.calls.find((call: any[]) => call[0] === 'watch_game');
  ...
  it('should handle leave_spectate', async () => {
    const leaveCall = (socket.on as jest.Mock).mock.calls.find((call: any[]) => call[0] === 'leave_spectate');
  ...
  it('should handle get_live_games and emit empty array if none found', async () => {
    const getLiveCall = (socket.on as jest.Mock).mock.calls.find((call: any[]) => call[0] === 'get_live_games');
