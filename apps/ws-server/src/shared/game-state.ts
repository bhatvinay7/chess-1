...
export async function getActiveGameId(userId: string): Promise<string | null> {
  ...
  const futureEntries = await redisClient.zRange(`matchmaking:gameId:${userId}`, nowMs, "+inf", { BY: 'SCORE' });
...
export async function removeGameFromSchedule(userId: string, gameId?: string): Promise<void> {
  if (!gameId) return;

  try {
    const members = await redisClient.zRange(`matchmaking:gameId:${userId}`, 0, -1);
    const target = members.find(m => m.startsWith(gameId + ":") || m === gameId);
    if (target) {
