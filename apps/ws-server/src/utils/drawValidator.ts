...
async function getAndValidateGameState(
  ...
  const redisValues = await redisClient.hmGet(`game:state:${game_id}`, requiredFields);
  ...
  if (game_state === "GAME_OVER" || game_state === "DRAW") {
    return {
      error: { message: "Current game is not in progress,Already complted", action: "not_allowed" }
    };
  }
  ...
  const elapsedSeconds = initialMoveTime > 0 ? currentTimeStamp - initialMoveTime : 0;
  ...
  const isTimedOut = (activeTurn === "w" && whiteTimeRemaining === 0) || (activeTurn === "b" && blackTimeRemaining === 0);
  if (isTimedOut) {
    return {
  ...
  requiredFields.forEach((field, idx) => {
    if (field === "player1_rating") rawState["player1_rating"] = redisValues[idx]!;
    else if (field === "player2_rating") rawState["player2_rating"] = redisValues[idx]!;
...
export async function offerDraw(
  ...
  const validation = await getAndValidateGameState(game_id, fieldsNeeded, "Draw can not be offered, game is over!");
  if (validation.error) return validation.error;

  const { activeTurn, rawState } = validation;
  
  const activeTurnPlayerId = activeTurn === "w" ? rawState.white_player_id : rawState.black_player_id;
  if (activeTurnPlayerId !== userId) {
    return { message: " current play user can offer draw ", action: "not_allowed" };
  }
  ...
  const payload: OfferDrawPayload = {
    ...
    username: isUserPlayer1 ? rawState.player1_username! : rawState.player2_username!,
    profile_image_url: isUserPlayer1 ? rawState.player1_profile_image_url! : rawState.player2_profile_image_url!,
...
export async function claimDraw(
  ...
  const hasDrawOffer = await redisClient.exists(`game:draw:${game_id}:${userId}`);
  if (!hasDrawOffer) {
    return { message: "No pending draw offer found to claim", action: "not_allowed" };
  }

  // Validate game is still active and clocks haven't expired
  const validation = await getAndValidateGameState(
    game_id,
    ["current_fen", "last_move_time", "black_player_left_time",
     "white_player_left_time", "game_state"],
...
export async function getPendingDrawOffer(
  ...
  const stored = await redisClient.hGet(`game:draw:${game_id}:${userId}`, "payload");
