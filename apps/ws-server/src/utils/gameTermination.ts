...
export async function fetchGameTerminationState(gameId: string): Promise<FullGameState> {
  const vals = await redisClient.hmGet(
    `game:state:${gameId}`,
    [...GAME_TERMINATION_FIELDS],
  );
  const [
    currentFen, gameState, lastMoveTime,
    whitePlayerId, blackPlayerId,
    player1Id, player2Id,
    player1Rating, player2Rating,
    player1ProfileImageUrl, player2ProfileImageUrl,
    player1Username, player2Username,
    whiteLeftTime, blackLeftTime,
    timeSlot, isRated, increment,
    tournamentId, tournamentType, roundId, groupId, matchId,
  ] = vals;
...
export async function terminateGame(
  ...
  if (isTournament) {
    ...
    if (player1Id && player2Id) {
      ...
      let p1Str = player1Rating && player1Rating.trim() !== "" ? player1Rating : "1200";
      let p2Str = player2Rating && player2Rating.trim() !== "" ? player2Rating : "1200";
