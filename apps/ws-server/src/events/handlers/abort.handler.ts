...
export class AbortHandler {
  ...
  private async onAbort(userId: string, gameId: string): Promise<void> {
    try {
      ...
      if (!isActive) {
        console.warn(`[abort] Game ${gameId} not active (status: ${state.gameState})`);
        return;
      }

      // 2. Verify caller is a player
      if (userId !== state.whitePlayerId && userId !== state.blackPlayerId) {
        console.warn(`[abort] User ${userId} is not a player in game ${gameId}`);
        return;
      }
      ...
      const whiteLeft = parseInt(state.whiteLeftTime ?? String(baseTimeSec), 10);
      const blackLeft = parseInt(state.blackLeftTime ?? String(baseTimeSec), 10);
      ...
      if (whiteUsed + blackUsed > 8) {
        console.warn(`[abort] Game ${gameId} elapsed time > 8s, cannot abort. w=${whiteUsed} b=${blackUsed}`);
        return;
      }
      ...
      this.io.to(`game:${gameId}`).emit(
        "game_state",
        buildTerminationData(state, "ABORTED", null),
      );
